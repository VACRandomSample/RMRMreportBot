...
class CallbackHandlers {
  ...
  registerCallbacks() {
    ...
    // Wizard callbacks
    this.bot.action(
      "category_punishments",
      this.handleCategoryPunishments.bind(this)
    );
  ...
  async handleCurrentSettings(ctx) {
    ...
    const hasToken = settings.yandexToken
      ? "✅ Установлен"
      : "❌ Не установлен";
  ...
  async handleCategoryPunishments(ctx) {
    ...
    const fileName = `punishment_${Date.now()}_${Math.random()
      .toString(36)
      .substring(7)}.jpg`;
    ...
    try {
      ...
      const saved = await this.savePhotoToYandex(
        userId,
        state.filePathLocal,
        remotePath
      );
  ...
  async handleCategoryMPHelp(ctx) {
    ...
    const fileName = `mp_help_${Date.now()}_${Math.random()
      .toString(36)
      .substring(7)}.jpg`;
  ...
  async sendStep3(ctx, userId, eventType) {
    ...
    try {
      const unfinishedEvents = await this.eventManager.getUnfinishedEvents(
        userId,
        remoteFolderPath
      );
  ...
  async sendMPStageStep(ctx, userId) {
    ...
    try {
      const unfinishedMPs = await this.eventManager.getUnfinishedEvents(
        userId,
        remoteFolderPath
      );
  ...
  async saveEventPhoto(ctx, userId, stage) {
    ...
    try {
      ...
      if (stage === "start") {
        ...
        eventNumber = await this.eventManager.getNextEventNumber(
          userId,
          remoteFolderPath
        );

        // Check if start already exists
        const eventExists = await this.eventManager.checkEventExists(
          userId,
          remoteFolderPath,
          eventNumber
        );
        ...
        const pendingEvent = this.stateManager.getPendingEvent(
          userId,
          eventType
        );
        ...
        if (pendingEvent) {
          ...
          const foundEventNumber = await this.eventManager.findEventWithoutEnd(
            userId,
            remoteFolderPath
          );
  ...
  async saveMPPhoto(ctx, userId, stage) {
    ...
    try {
      ...
      if (stage === "start") {
        // Get next MP number
        mpNumber = await this.eventManager.getNextMPNumber(
          userId,
          remoteFolderPath
        );
        ...
        if (pendingMP) {
          ...
          // Check if end already exists
          const files = await this.yandexDisk.listFiles(
            userId,
            remoteFolderPath
          );
          ...
          // Find MP without end
          const foundMPNumber = await this.eventManager.findEventWithoutEnd(
            userId,
            remoteFolderPath
          );
