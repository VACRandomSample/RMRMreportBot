...
const { generateFileName } = require("@org/core");

class PhotoHandlers {
  constructor(bot, fileManager, stateManager, callbackHandlers) {
    this.bot = bot;
  ...
  async handlePhoto(ctx) {
    ...
    try {
      ...
      const filePathLocal = await this.fileManager.downloadFile(
        fileUrl,
      ...
      // Start wizard
      await this.callbackHandlers.sendStep1(ctx, userId);
    } catch (error) {
  ...
  async handleDocument(ctx) {
    ...
    try {
      ...
      const ext =
        path.extname(document.file_name) ||
