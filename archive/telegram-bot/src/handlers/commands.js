...
class CommandHandlers {
  ...
  async handleAuth(ctx) {
    const authUrl = `https://oauth.yandex.ru/authorize?response_type=code&client_id=${
      config.yandex.clientId
    }&redirect_uri=${encodeURIComponent(config.yandex.redirectUri)}`;
  ...
  async handleSyncEvents(ctx) {
    ...
    try {
      ...
      for (const eventType of eventTypes) {
        ...
        try {
          const summary = await this.eventManager.getEventSummary(
            userId,
            remoteFolderPath
          );
