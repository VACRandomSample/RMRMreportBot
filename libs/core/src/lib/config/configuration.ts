  ...
  return {
    ...
    yandex: {
      ...
      clientSecret: (process.env.YANDEX_CLIENT_SECRET ||
        "ВАШ_CLIENT_SECRET") as string,
      redirectUri: (process.env.YANDEX_REDIRECT_URI ||
        "https://oauth.yandex.ru/verification_code") as string,
    ...
    photosDir: (process.env.PHOTOS_DIR ||
      path.join(projectRoot, "photos")) as string,
    settingsFile: (process.env.SETTINGS_FILE ||
      path.join(projectRoot, "user_settings.json")) as string,
    ...
    fileRetentionTime: parseInt(
      process.env.FILE_RETENTION_TIME || "3600000",
      10
    ),
